...
int JWMonster::JWcount = 0;

JWMonster::JWMonster(string _name, int _atk, int _def, int _exp, int _money, int _max_hp, int _max_mp)
...
BaseMonster * JWMonster::unserialize(string text_data)
{
	int cursor = 0;
	...
	if (text_data_name != "class JWMonster")
	{
		return nullptr;
	}
	...
	BaseMonster* ptr = new JWMonster(const_name, const_attack, const_defense, const_exp, const_money, const_max_hp, const_max_mp);
	ptr->setHp(old_hp);
	ptr->setMp(old_mp);
