...
int ZombieMonster::Zombiecount = 0;

ZombieMonster::ZombieMonster(string _name, int _atk, int _def, int _exp, int _money, int _max_hp, int _max_mp)
...
BaseMonster * ZombieMonster::unserialize(string text_data)
{
	int cursor = 0;
	...
	if (text_data_name != "class ZombieMonster")
	{
		return nullptr;
	}
	...
	BaseMonster* ptr = new ZombieMonster(const_name, const_attack, const_defense, const_exp, const_money, const_max_hp, const_max_mp);
	ptr->setHp(old_hp);
	ptr->setMp(old_mp);
